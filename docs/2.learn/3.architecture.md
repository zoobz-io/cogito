...
Implement the `Memory` interface for your storage backend. A typical implementation requires:

### Thought Storage

Store thought records with:
- `id` - Unique identifier (UUID recommended)
- `intent` - Purpose of the reasoning chain
- `trace_id` - Unique trace for observability
- `parent_id` - Optional parent thought reference
- `task_id` - Optional task grouping
- `created_at`, `updated_at` - Timestamps

### Note Storage

Store note records with:
- `id` - Unique identifier
- `thought_id` - Foreign key to thought
- `key` - Lookup key
- `content` - Text content
- `metadata` - Structured extension (JSON)
- `source` - Origin primitive
- `created` - Timestamp
...
4. **Connection Pooling** - Use connection pooling in your Memory implementation
